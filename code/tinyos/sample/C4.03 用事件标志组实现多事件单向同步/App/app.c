...
void task1Entry (void *param) {
    for (;;) {
        ...
        tTaskDelay(1);
				...
				tFlagGroupNotify(&syncFlagGroup, 1, 0x1);
    }
}
...
void task2Entry (void *param) {
    for (;;) {
        ...
        task2Flag = 0;
        tTaskDelay(1);
				tFlagGroupNotify(&syncFlagGroup, 1, 0x2);
    }
}
...
void task3Entry (void *param) {
    for (;;) {
        ...
        tTaskDelay(1);
				...
				tFlagGroupNotify(&syncFlagGroup, 1, 0x4);
...
void task4Entry (void *param) {
    for (;;) {
        uint32_t resultFlag = 0;
				...
				tFlagGroupWait(&syncFlagGroup, TFLAGGROUP_SET_ALL | TFLAGGROUP_CONSUME, (0x1 | 0x2 | 0x4), &resultFlag, 0);
				xprintf("Event wait OK!\n");
		}
}
...
void tInitApp (void) {
    halInit();
		...
		tFlagGroupInit(&syncFlagGroup, 0);
