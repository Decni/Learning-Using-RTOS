...
void task1Entry (void *param) {
    for (;;) {
        ...
        tTaskDelay(1);
				...
				tMboxNotify(&syncMbox, (void *)1, tMBOXSendNormal);
    }
}
...
void task2Entry (void *param) {
    for (;;) {
        ...
        task2Flag = 0;
        tTaskDelay(1);
				tMboxNotify(&syncMbox, (void *)2, tMBOXSendNormal);
    }
}
...
void task3Entry (void *param) {
    for (;;) {
        ...
        task3Flag = 0;
        tTaskDelay(1);
				tMboxNotify(&syncMbox, (void *)3, tMBOXSendNormal);
    }
}
...
void task4Entry (void *param) {
    for (;;) {
				...
				tMboxWait(&syncMbox, &msg, 0);
				if (msg != 0) {
						xprintf("Hello task %d\n", (uint32_t)msg);
				}
    }
}
...
void tInitApp (void) {
    halInit();
		...
		tMboxInit(&syncMbox, mboxBuffer, 20);
