...
int globalVar = 0;      // 被任务、两个中断所共享的全局变量
 ...
 * 中优先级中断处理函数
 */
void IRQMiddleHandler (void) {
    int temp;
    tTaskCritical_t critical;
    
    critical = tTaskEnterCritical();
    temp = globalVar + 2;
    interruptByIRQ(IRQ_PRIO_HIGH);          // 回写前被高优先级打断
    globalVar = temp;
...
void task1Entry (void *param) {
    interruptByIRQ(IRQ_PRIO_MIDDLE);

    xprintf("IRQ Middle: %d", globalVar);
    for (;;) {
        task1Flag = 1;
        tTaskDelay(1);
        task1Flag = 0;
...
void tInitApp (void) {
    ...
    interruptEnable(IRQ_PRIO_MIDDLE, 1);
