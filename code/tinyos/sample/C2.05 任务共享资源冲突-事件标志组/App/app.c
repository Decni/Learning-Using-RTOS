...
static tSem shareCountSem;
static tFlagGroup shareCountFlagGroup;
...
#if 1
    tFlagGroupWait(&shareCountFlagGroup, TFLAGGROUP_SET_ANY|TFLAGGROUP_CONSUME, 0x1, &resultFlag, 0);
    shareCountIncrease(1, 1);    
    xprintf("Task1 S1:%d/%d\n", shareCountGet(), realCount);
    tFlagGroupNotify(&shareCountFlagGroup, 1, 0x1);
#else
...
void tInitApp (void) {
    ...
    tFlagGroupInit(&shareCountFlagGroup, 0x1);
